...
@interface ScrollViewRight()<UIScrollViewDelegate>
{
    CGFloat kcellW;
...
@implementation ScrollViewRight
...
-(void)reloadData{
    ...
    if (count==1) {
        ...
        UIImageView * imageV2 = [self.delegate cellImageWithIndex:1];
        imaV2.image = imageV2.image;
        imaV2.tag=1;
        ...
        if (count==2) {
            ...
            UIImageView * imageV3 = [self.delegate cellImageWithIndex:2];
            imaV3.image = imageV3.image;
            imaV3.tag=2;
        }
...
-(void)tapA:(UITapGestureRecognizer *)tapsender{
    ...
    isdrag = YES;
    [scrollview setContentOffset:CGPointMake(kcellW*2, 0) animated:YES];
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
   // NSLog(@"scrollview3-%f",scrollView.contentOffset.x );
    if (isdrag) {
        
        CGFloat x = scrollView.contentOffset.x;
        CGFloat cut = x-offest;
        [self.delegate RightscrollViewing:cut :NO];
        offest = scrollView.contentOffset.x;
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    if (isdrag) {
        ...
        if (offest1>x) {
            [self.delegate RightscrollViewing:cut :YES :NO];
            [self imaVwithImage:NO];
        }else if (offest1<x){
            [self.delegate RightscrollViewing:cut :YES :YES];
            [self imaVwithImage:YES];
        }else{
            [self.delegate RightscrollViewing:cut :YES];
